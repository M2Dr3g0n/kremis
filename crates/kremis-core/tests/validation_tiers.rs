...
mod t1_deterministic_edges {
    ...
    fn same_signals_same_edges() {
        ...
        assert_eq!(graph1.get_edge(a1, b1), graph2.get_edge(a2, b2));
    }
    ...
    fn weight_increments_exact() {
        ...
        assert_eq!(graph.get_edge(a, b), Some(EdgeWeight::new(10)));
    }
    ...
    fn edge_creation_reproducible() {
        ...
        for _ in 0..2 {
            ...
            for i in 0..9 {
                graph.increment_edge(nodes[i], nodes[i + 1]);
            }
...
mod t2_single_hop {
    ...
    /// T2.2: Missing node returns empty iterator.
    #[test]
    fn missing_node_empty() {
        let graph = Graph::new();
        let fake_node = NodeId(999);

        let neighbors: Vec<_> = graph.neighbors(fake_node).collect();
    ...
    fn neighbor_order_deterministic() {
        ...
        graph.insert_edge(a, d, EdgeWeight::new(1));
        graph.insert_edge(a, b, EdgeWeight::new(1));
        graph.insert_edge(a, c, EdgeWeight::new(1));
    ...
    fn lookup_returns_correct() {
        let mut graph = Graph::new();
        let node = graph.insert_node(EntityId(42));

        let result = graph.lookup(node);
    ...
    fn lookup_missing_returns_none() {
        let graph = Graph::new();
        let result = graph.lookup(NodeId(999));
...
mod t3_multi_hop {
    ...
    fn bfs_correct_paths() {
        ...
        let artifact = graph.traverse(a, 3).expect("should traverse");
    ...
    fn traversal_depth_respected() {
        ...
        for i in 0..9 {
            graph.insert_edge(nodes[i], nodes[i + 1], EdgeWeight::new(1));
        }

        // Depth 2 should only reach first 3 nodes
        let artifact = graph.traverse(nodes[0], 2).expect("traverse");
    ...
    fn artifact_complete_trace() {
        ...
        let artifact = graph.traverse(a, 2).expect("traverse");
    ...
    fn cyclic_graphs_handled() {
        ...
        let artifact = graph.traverse(a, 10);
    ...
    fn missing_path_returns_none() {
        ...
        let path = graph.strongest_path(a, b);
        assert!(path.is_none());
    }
}
